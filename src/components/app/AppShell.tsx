...
export function AppShell({ title, subtitle, children, action }: { title: string; subtitle?: string; children: ReactNode; action?: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-void">
      <aside
        className={`fixed inset-y-0 left-0 z-40 hidden flex-col border-r border-border bg-raised/60 backdrop-blur-xl transition-all duration-500 md:flex ${
      ...
      >
        ...
        <nav className="flex-1 space-y-1 px-3 py-2">
          {nav.map((n) => {
            ...
            return (
              <Link key={n.to} to={n.to} className="relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground transition-colors hover:text-foreground">
                {active && (
                  <motion.span
                    layoutId="nav-active"
                    className="absolute inset-0 rounded-xl border border-mint/25 bg-mint/12"
                    transition={{ type: "spring", stiffness: 400, damping: 34 }}
                  />
                )}
                <n.icon className={`relative size-[18px] shrink-0 ${active ? "text-mint" : ""}`} />
      ...
      <div className={`transition-all duration-500 ${collapsed ? "md:pl-[72px]" : "md:pl-[236px]"}`}>
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-void/80 px-4 backdrop-blur-xl md:px-6">
          <button
            onClick={() => setOpen(true)}
            className="flex flex-1 items-center gap-2 rounded-xl border border-border bg-surface/70 px-3 py-2 text-sm text-muted-foreground transition-colors hover:border-border-active md:max-w-sm"
          >
          ...
          <Link to="/app/alerts" className="rounded-xl border border-border bg-surface/70 p-2 text-muted-foreground hover:text-foreground" aria-label="Alerts">
            <Bell className="size-4" />
          </Link>
          <DropdownMenu>
            <DropdownMenuTrigger className="grid size-9 place-items-center rounded-xl border border-border bg-surface/70 text-muted-foreground hover:text-foreground">
        ...
        >
          <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
            <div>
              <h1 className="font-display text-2xl font-semibold md:text-[28px]">{title}</h1>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-around border-t border-border bg-raised/90 px-2 py-2 backdrop-blur-xl md:hidden">
        {nav.slice(0, 5).map((n) => {
          const active = n.to === "/app" ? path === "/app" : path.startsWith(n.to);
          return (
            <Link key={n.to} to={n.to} className={`flex flex-col items-center gap-1 rounded-lg px-3 py-1.5 text-[10px] ${active ? "text-mint" : "text-muted-foreground"}`}>
